...
## Branchlet - A CLI tool to create a manage worktrees

[Branchlet](https://github.com/raghavpillai/branchlet) is an interactive CLI tool with an easy to user interface which you can use to create and manage workrees.
It's most notable features include ability to auto copy user specified files like env files to your new worktrees. This way you do not need to manually copy them. New worktrees would be instantly runnable.
Apart from copying files, you can also specify post creation commands such as `npm install`. Using such a tool makes managing multiple worktrees extremely easy.
